...
public final class FirstPartTasks {
    ...
    public static List<Album> sortedFavorites(Stream<Album> albums) {
        ...
        return albums
                .filter(album -> album.getTracks().stream()
                        .map(Track::getRating)
                        .anyMatch(rating -> rating > threshold)
                )
    ...
    public static Map<Artist, List<String>> groupByArtistMapName(Stream<Album> albums) {
        return albums.collect(Collectors.groupingBy(
                Album::getArtist, Collectors.mapping(Album::getName, Collectors.toList())));
    }
    ...
    public static List<Album> sortByAverageRating(Stream<Album> albums) {
        return albums
                .sorted(Comparator.comparingDouble(album -> ((Album) album).getTracks().stream()
                        .mapToInt(Track::getRating)
                        .average()
                        .orElse(0)
                ).reversed())
